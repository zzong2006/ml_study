...
def main():
    ...
    coef2column = {coef: column for coef, column in zip(estimator.coef_, train_dataset.X.columns) if coef != 0}
    for idx, (coef, column) in enumerate(sorted(coef2column.items(), key=lambda x: abs(x[0]), reverse=True)):
